  ...
  if (isset($_POST['update_flight_but'])) {
    ...
    if (!isset($_POST['flight_id']) || !isset($_SESSION['adminId'])) {
    ...
    header('Location: ../../admin/flight.edit.php?error=same'); 
              exit();
    }
    ...
    if ($flag == false) {
      header('Location: ../../admin/flight.edit.php?error=destless');
      exit();
    } else {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          $admin_id = $_SESSION['adminId'];
          mysqli_stmt_bind_param($stmt, 'isssssisi', $admin_id, $arrival, $departure, $arr_city, $dep_city, $airline_name, $price, $status, $flight_id);
          mysqli_stmt_execute($stmt);
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
        header('Location: ../../admin/all_flights.php?flight=updated'); 
        exit();
