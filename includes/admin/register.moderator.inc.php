...
if(isset($_POST['signup_submit'])) {
    ...
    if(!filter_var($email_id,FILTER_VALIDATE_EMAIL)) {
        ...
        $username_sql = 'SELECT username FROM moderator WHERE username=?';
        $email_sql = 'SELECT email FROM moderator WHERE email=?';
        ...
        if(!mysqli_stmt_prepare($stmt,$username_sql)) {
            ...
            if($username_check > 0) {
                header('Location: ../admin/register.moderator.php?error=usernameexists');
                exit();                  
            } else {
                $stmt = mysqli_stmt_init($conn);
                if(!mysqli_stmt_prepare($stmt,$email_sql)) {
                    header('Location: ../admin/register.moderator.php?error=sqlerror');
                    exit();            
                } else {
                    ...
                    if($email_check > 0) {
                        header('Location: ../admin/register.moderator.php?error=emailexists');
                        exit();   
                    } else {
                        $sql = 'INSERT INTO moderator (username, email, password)
                         VALUES (?, ?, ?)';
                        $stmt = mysqli_stmt_init($conn);
                            ...
                            if(!mysqli_stmt_prepare($stmt,$sql)) {
                                ...
                                if($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    if($pwd_check == false) {
                                        ...
                                        header('Location: ../../admin/register.moderator.php?register=success');
                                        exit();                  
                                    } else {
