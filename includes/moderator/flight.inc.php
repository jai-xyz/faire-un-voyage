...
if(isset($_POST['flight_but']) && isset($_SESSION['modId'])) {
    ...
    if($dep_city===$arr_city || $arr_city==='To' || $arr_city==='From') {
      header('Location: ../../moderator/flight.php?error=same');
      exit();
    }
    ...
    if($flag == false) {
      header('Location: ../../moderator/flight.php?error=destless');
      exit();
    } else {
      ...
      if($row = mysqli_fetch_assoc($result)) {
        ...
        $sql = "INSERT INTO Flight(moderator_id,arrivale,departure,Destination,source,
          airline,Seats,status,Price) VALUES (?,?,?,?,?,?,?,?,?)";
          
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt,$sql)) {
          header('Location: ../../moderator/flight.php?error=sqlerr1');
          exit();          
        } else {      
          $mod_id = $_SESSION['modId'];  
          
          mysqli_stmt_bind_param($stmt,'isssssisi',$mod_id,$arrival,$departure,$arr_city,$dep_city,$airline_name,$seats,$status,$price);            
          mysqli_stmt_execute($stmt); 
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);  
        header('Location: ../../moderator/all_flights.php?flight=added');
        exit();
      } else {
        header('Location: ../../moderator/flight.php?error=sqlerr');
        exit();
      }
    }
} else {
